---
name: discuss
description: Use when exploring a feature idea before committing, or revisiting a parked one. Triggers — "engineer plugin -> discuss skill", "engineer plugin -> discuss skill <slug>", "I have an idea about", "should we build", "let's think about".
---
...
- **Fresh** — `engineer plugin -> discuss skill`, no argument. New exploration.
- **Continue** — `engineer plugin -> discuss skill <slug>`. Resume a parked feature.

**Not for:** feature-work prep on an already-Ready feature (`prime-context`), or editing a committed feature (`feature-edit`).

## Workflow

1. **Resolve + load** — resolve the methodology root + manifest via `${PLUGIN_ROOT}/scripts/dae_resolve.py` (see `references/resolving.md`). Load `CHARTER.md` and the last ~20 lines of `.engineer/discussions.log`. With a slug arg: also load that feature's `feature.md` + prior `handoffs/*-discuss.md`; reject if its status is `ready`/`in-progress` (→ `feature-edit`) or `done`.
