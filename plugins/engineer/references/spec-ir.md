...
```
python3 ${PLUGIN_ROOT}/scripts/dae_gherkin.py SPEC_MD [OUT_JSON]
```

`atdd` plugin `atdd` skill (Checkpoint 3) produces `spec.md`; the parser produces
...
- **IR mutator** (`dae_gherkin_mutate.py`) — alters example values in the IR, one cell per mutation; the project regenerates tests from each mutated IR and expects them to fail. Checks the acceptance tests are *wired to the app*. **Distinct from `atdd` plugin `mutate` skill**, which mutates *source code* to check the unit suite (Checkpoint 8 Hardening) — different mutator, different purpose.
