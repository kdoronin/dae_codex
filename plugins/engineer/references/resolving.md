...
```
python3 ${PLUGIN_ROOT}/scripts/dae_resolve.py [START_DIR]
```
...
- **1** — manifest found but **invalid**. Surface `errors` to the user and stop; recommend `engineer plugin -> consistency-check skill --project` or fixing the manifest. Don't proceed against a broken manifest.
- **2** — **no manifest found**. The project isn't DAE-onboarded. Stop and point the user to `engineer plugin -> onboard skill`.
- **3** — usage error.
