...
description: >-
  ...
  available as the atdd plugin's main skill. Enforces the Acceptance Test Driven
...
Before Step 1, create one Codex plan item per step of this workflow (Steps 1–7),
...
the project's `AGENTS.md` (or create one if it doesn't exist). This
ensures Codex CLI understands the ATDD setup in every session:
...
Adapt the workflow entrypoints and paths to match the project's language and
test framework. The pipeline-builder agent generates this AGENTS.md
