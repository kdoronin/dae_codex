...
1. Run engineer plugin -> consistency-check skill — confirm the artifacts agree.
2. Run crap-analyzer skill — CRAP + coverage on the changed code (Checkpoint 7).
3. Run mutation testing per the CHARTER's mutation policy (Checkpoint 8).
   ...
   skip it. Run atdd plugin -> mutate skill, then atdd plugin -> kill-mutants skill for real survivors,
