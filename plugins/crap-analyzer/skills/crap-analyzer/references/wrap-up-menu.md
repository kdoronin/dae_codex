...
After the CRAP report and per-finding proposals are on screen, present an actionable menu via `prompt the user` so the user can dispatch work in one step. Do this every time, even with a single finding.
