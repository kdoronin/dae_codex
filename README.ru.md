...
Плагин `atdd` включает advisory, non-destructive hooks:

- `PreToolUse`: предупреждает перед source writes, если acceptance specs не найдены.
- `PostToolUse`: напоминает, что нужно проверить и acceptance, и unit streams.

Source-of-truth plugin hook artifact: `plugins/atdd/hooks/hooks.json`. Текущие версии Codex CLI корректно устанавливают plugin skills, но plugin-local hook manifests пока не всегда надежно подключаются к runtime hook engine. Поэтому one-command installer создает user-level bridge `~/.codex/hooks.json`, который указывает на установленные ATDD hook scripts. Для локальной разработки marketplace в репозитории также есть `.codex/hooks.json`.

Старый completion hook не имел прямого one-to-one соответствия Codex event, поэтому reminder привязан к `PostToolUse`. Hook scripts предпочитают `PLUGIN_ROOT` и `PLUGIN_DATA`; legacy environment names используются только как compatibility fallbacks внутри этих scripts.

Когда guardrail срабатывает, он выводит Codex hook context, а не удаляет и не переписывает файлы. Implementation-before-spec guard является warning, потому что текущая семантика Codex hooks не делает этот plugin универсальным hard blocker.
